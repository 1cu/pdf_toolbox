...
def test_pptx_to_images_e2e(monkeypatch, simple_pptx, sample_pdf, tmp_path):
    ...
    result_dir = pptx_to_images(
        simple_pptx, out_dir=str(output_dir), image_format="PNG"
    )
...
def test_pptx_to_pdf_e2e(monkeypatch, simple_pptx, sample_pdf, tmp_path):
    class DummyRenderer(BasePptxRenderer):
        name = "dummy"
            ...
            with fitz.open(sample_pdf) as source:
                ...
                for page_no in selected:
                    result_doc.insert_pdf(
                        source, from_page=page_no - 1, to_page=page_no - 1
                    )
