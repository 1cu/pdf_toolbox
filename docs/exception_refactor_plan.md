...
* Group the 137 inline exceptions by file to understand their shared causes.
...
* **L165, L201 – `type: ignore[attr-defined]` (missing Qt enums)**: Ship custom stub augmentations (`qt-stubs.pyi`) that expose the missing enum members (`QFormLayout.FieldGrowthPolicy`, `QToolButton.ToolButtonPopupMode`). PySide 6.6+ already supports stub packages via `typing_extensions.override`; adding the definitions removes the ignores.
...
2. **Adopt configuration dataclasses for actions/renderers**: A shared `RenderOptions` (and action-specific options) object removes 22 separate `PLR0913`/`PLR0912` suppressions in actions, renderers, and tests.
3. **Improve test coverage of exceptional paths**: Investing in faked codecs, dependency toggles, and Windows CI coverage allows us to drop 18 `pragma: no cover` markers.
4. **Introduce protocols/stub files for Qt integration**: Replacing runtime attribute hacks with well-typed protocols and `.pyi` files resolves 20 Qt-related ignores and naming suppressions.
